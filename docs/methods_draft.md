...
Meta2Data is an end-to-end command-line pipeline for the automated retrieval, processing, and cross-study integration of 16S rRNA amplicon sequencing data from public repositories, implemented in Python 3 and Bash. The pipeline is organised as three sequential modules—MetaDL, AmpliconPIP, and ggCOMBO—chained together by a unified bash orchestrator and a JSON-based checkpoint/resume system that enable consecutive multi-stage processing without manual intervention between stages. Meta2Data is provided as open-source software deployable on any Linux system, including HPC clusters, by cloning the repository and adding its `bin/` directory to `PATH`; AmpliconPIP and ggCOMBO additionally require activation of the QIIME2 amplicon distribution 2024.10 conda environment, while the required native binaries (vsearch 2.30.0 and fastp 0.24.0) and Python dependencies are detected and auto-installed on first invocation. Network connectivity is required on first invocation to query the NCBI Entrez and CNCB APIs, download SRA/CRR reads, and fetch reference databases; downloaded taxonomy databases are cached locally and can be reused offline on subsequent runs via the `--db` parameter. The Meta2Data source code and documentation are freely available at https://github.com/LinyangSun/Meta2Data.
